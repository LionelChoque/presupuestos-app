...
export function convertCsvToBudgets(csvData) {
            ...
            complete: (results) => {
                try {
                    ...
                    budgetMap.forEach((rows, budgetId) => {
                        ...
                        const { tipoSeguimiento, accion, prioridad, diasRestantes, diasTranscurridos, alertas, esLicitacion, contacto } = calculateBudgetDetails(rows);
...
export function compareBudgets(existingBudgets, newBudgets, options) {
    const result = {
        added: 0,
        updated: 0,
        deleted: 0,
...
export async function processImportedCsv(csvData, existingBudgets, options) {
    try {
